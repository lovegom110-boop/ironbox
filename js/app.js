...
(function () {
  ...
  async function loadDay(date) {
    ...
    // 빈 날이면, 직전 기록일의 '미완료' 할 일을 이월
    if (day.tasks.length === 0) {
      const all = await Store.getAllDays();
      const prev = all.filter((d) => d.date < date && d.tasks.length).sort((a, b) => (a.date < b.date ? 1 : -1))[0];
      if (prev) {
        const carry = prev.tasks.filter((t) => !t.done).map((t) => { const n = Store.newTask(t.text); n.category = t.category; return n; });
        if (carry.length) { day.tasks = carry; await Store.saveDay(day); }
      }
    }
    state.day = day;
