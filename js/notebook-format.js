(function (global) {
  ...
  function toggleHeading(markdown, selection, level) {
    const lines = String(markdown || "").split("\n");
    const startLine = Math.max(1, Math.min(selection[0][0], selection[1][0]));
    const endLine = Math.max(startLine, Math.max(selection[0][0], selection[1][0]));
    const selected = lines.slice(startLine - 1, endLine);
    const candidates = selected.filter((line) => line.trim());
    const sameLevel = level > 0 && candidates.length > 0
      && candidates.every((line) => new RegExp("^\\s*#{" + level + "}\\s+").test(line));
    ...
    for (let i = startLine - 1; i < endLine && i < lines.length; i += 1) {
      ...
      lines[i] = level === 0 || sameLevel ? indent + content : indent + "#".repeat(level) + " " + content;
    }
  ...
  global.NotebookFormat = { toggleLinePrefix, toggleHeading };
