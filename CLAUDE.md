...
### Fixed (오늘의 메모 — 입력값과 표시값이 달라지던 데이터 손상 + 줄바꿈 깨짐)
- **증상**: 「오늘의 메모」에 `2025. 10. 130,364 …`처럼 숫자로 시작하는 여러 줄(날짜별 요금표 등)을 저장하면, 편집(EasyMDE)에선 입력 그대로지만 **카드 렌더에서 연도가 2025·2026·2027·2028…로 자동 증가**해 입력값(2025·2025·2025·2026…)과 **표시값이 달라짐**. 줄바꿈도 한 문단으로 뭉침.
- **원인**: `notes.js`의 `mdToSafeHtml`가 `marked.parse()`를 옵션 없이 호출. `2025. `(숫자+점+공백)을 마크다운이 **순서목록 마커**로 해석 → `<ol start="2025">`가 항목마다 **번호를 자동 재계산**(2025,2026,2027…)하고, 월(`10.`)은 중첩 `<ol>`로 빠지며 값/줄이 손상. (기본 `breaks:false`라 줄바꿈도 공백으로 붕괴.)
- **수정**: `notes.js`에서 **marked 목록 토크나이저를 1회 비활성화**(`marked.use({ tokenizer:{ list(){return undefined;} } })`) → `2025. 10. …` 줄이 목록으로 안 바뀌고 **입력 그대로** 표시. **굵게·기울임·제목·링크·인용 등 다른 마크다운은 유지**. 추가로 `parse(md,{breaks:true,gfm:true})`로 줄바꿈(`<br>`) 보존. 실제 marked+CSS 렌더로 전/후 재현·검증(연도 2025→2032 손상 → 입력 그대로 복원). ⚠️ 트레이드오프: 메모에서 `1.`·`-`로 시작해도 자동 번호/글머리 목록으로는 안 바뀜(입력 그대로 표시) — 구조화 문서는 상단 「노트」 사용. SW `v46 → v47`. (`js/notes.js`, `sw.js`)
