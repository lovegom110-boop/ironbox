...
## 9. 확정 전 결정사항 (구현 착수 전 답할 것)
- [ ] **오프라인 전략**: A(Firestore 단독) vs B(하이브리드) — *권장 A*
- [ ] **인증 범위**: Google만 vs Google+이메일 — *권장 Google만*
- [ ] **Calendar OAuth 통합 여부**: Phase 1에서 분리 유지 OK? — *권장 분리*
- [ ] **기존 디스크 파일 백업 기능 유지 여부** — *권장 유지(안전망)*
